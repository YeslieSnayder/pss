...
TEST_F(DriverGetTest, ExampleData) {
    Document driver_doc;
    ...
    try {
        ...
        ASSERT_EQ(0, 1);    // Continue tests, if there was exception
    }

//    EXPECT_TRUE(driver == *test_driver);
    ...
    EXPECT_EQ(driver.getStatus(), test_driver->getStatus());
