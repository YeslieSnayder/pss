...
string getAcceptableRoles();

vector<string> acceptableRoles = {"student", "lab employee", "professor", "director", "admin", "guest"};
...
void work(User *user, vector<User*> &users) {
    ...
    while (true) {
        cout << endl << "You can enter the room (1), "
             ...
             << (isAdmin ? "print information about all user (5), "
                           "give access for user to the room (6), "
                           "set emergency situation (7), "
                           "reset emergency situation (8), " : "")
        ...
        } else if (command == "7" && isAdmin) {
            log("Attention! Emergency situation! Please, leave the building immediately using emergency exits!");
            Admin *admin = dynamic_cast<Admin*>(user);
            setEmergencySituation(admin, true);
        } else if (command == "8" && isAdmin) {
            Admin *admin = dynamic_cast<Admin*>(user);
            setEmergencySituation(admin, false);
            log("Alarm was deactivated!");
...
User* logIn(vector<User*> &users) {
    string role, name;
    cout << "Please, select one of the available roles (" << getAcceptableRoles() << "): ";
    getline(cin, role);
    while (!isRightRole(role)) {
        cout << "Available roles are " << getAcceptableRoles() << ": ";
        getline(cin, role);
...
User* signUp() {
    ...
    getline(cin, s);
    while (!isRightRole(s)) {
        cout << "Available roles are Guest, Student, Lab employee, Professor, Director, Admin: ";
        cin >> s;
    ...
    cin >> name;
    cout << "Please, enter your age" << ((role != "guest") ? " (age > 16): " : ": ");
    cin >> age;
    while (age <= 16 && role != "guest") {
    ...
    if (role == "student") {
        ...
        if (extraInfo.empty())
            ...
            return createUser(role, name, "", age, NO_LEVEL, course,
                              NO_LABORATORY, 0, false, "", 0, extraInfo);
        ...
        if (extraInfo.empty())
            ...
            return createUser(role, name, "", age, YELLOW, 0,
                              lab, workExperience, false, "", 0, extraInfo);
        ...
        getline(cin, password);
        if (extraInfo.empty())
            return createUser(role, name, "", age, SUPER_USER, 0, NO_LABORATORY, 0, false, "", 0,
                              "", password);
        else
            return createUser(role, name, "", age, SUPER_USER, 0, NO_LABORATORY, 0, false, "", 0,
                              extraInfo, password);
    } else if (role == "guest") {
        log("Each guest has to have hometown and their own group number.");
        log("Please, write your hometown:");
        string hometown;
        getline(cin, hometown);
        getline(cin, hometown);
        log("Please, write your group number:");
        int groupNumber;
        cin >> groupNumber;
        if (extraInfo.empty())
            return createUser(role, name, "", age, BLUE, 0,
                              NO_LABORATORY, 0, false, hometown, groupNumber);
        else
            return createUser(role, name, "", age, BLUE, 0,
                              NO_LABORATORY, 0, false, hometown, groupNumber, extraInfo=extraInfo);
    }
    ...
    for (User* user : users) {
        ...
        if (userName == name) {
            ...
            switch (level) {
                ...
                case BLUE:
                    if (role == "guest") return user->getId();
                case GREEN:
...
string getAcceptableRoles() {
    string s = "";
    for (int i = 0; i < acceptableRoles.size() - 1; i++) {
        s += acceptableRoles[i];
        s += ", ";
    }
    s += acceptableRoles[acceptableRoles.size() - 1];
    return s;
}
