...
vector<User*> getUsers() {
    ...
    for (int i = 0; i < doc.GetRowCount(); i++) {
        ...
        string hometown = obj[11];
        int groupNumber = (!obj[12].empty()) ? doc.GetCell<int>(12, i) : 0;

        // Create an user
        user = createUser(obj[0], name, surname, age, accessLevel, course, lab, workExperience,
                          isPhd, hometown=hometown, groupNumber=groupNumber,
                          extraInfo=extraInfo, password=password);
        if (user != nullptr)
...
vector<Room*> getRooms() {
    ...
    for (int i = 0; i < doc.GetRowCount(); i++) {
        ...
        int floor = (!obj[6].empty()) ? doc.GetCell<int>(6, i) : 0;
        if (obj[1] == Capacity_nms::toString(DIRECTOR_CABINET)) {
            continue;
        }

        // Create a room
        room = createRoom(type, number, fullness, isBooked, lab, nullptr, floor);
        if (room != nullptr)
...
vector<Room*> getRooms(vector<User*> &users) {
    ...
    for (int i = 0; i < doc.GetRowCount(); i++) {
        ...
        bool isBooked = obj[4] == "true";
        Laboratory lab = Laboratory_nms::toEnum(obj[5]);
        int floor = (!obj[7].empty()) ? doc.GetCell<int>(7, i) : 0;
        ...
        // Create a room
        room = createRoom(type, number, fullness, isBooked, lab, owner, floor);
        if (room != nullptr)
            rooms.push_back(room);
        else
            log(WARNING, "Problem with reading file with rooms. Type of room(" + obj[0] + ") wasn't found.");
