...
int main() {
    ...
    // Test values
    string name = "Ivan", surname = "Ivanov", extraInfo = "smt", password = "password", hometown = "Innopolis";
    uint age = 20, course = 4, workExperience = 10, groupNumber = 3;
    ...
    // Test #6
    user = new Admin(name, surname, age, extraInfo, password);
    result = (name + " " + surname) == user->getName()
             && age == user->getAge()
    ...
    // Test #7
    user = new Guest(name, surname, age, hometown, groupNumber, extraInfo);
    result = (name + " " + surname) == user->getName()
             && age == user->getAge()
             && Guest::ACCESS_LEVEL == user->getAccessLevel()
             && extraInfo == user->getExtraInfo()
             && hometown == ((Guest*) user)->getHometown()
             && groupNumber == ((Guest*) user)->getGroupNumber();
    if (!result) return 7;
}